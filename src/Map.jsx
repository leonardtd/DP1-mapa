...
import plantaPrincipalImg from './assets/plantaPrincipal.png'
import plantaSecundariaImg from './assets/plantaSecundaria.png'
import camionCisternaImg from './assets/camionCisterna.png'
import clientWarehouseImg from './assets/clientWarehouse.png'
import roadblockImg from './assets/roadblock.png'
...
const Map = (blockSize_p) => {
    ...
    const renderRoute = (p5,curNode,path,xFactor,yFactor) =>{
        ...
        for(var i=curNode; i<path.length-1;i++){
            p5.line(
                i==curNode ? path[i]['x']*blockSize+xFactor: path[i]['x']*blockSize, 
                i==curNode ? path[i]['y']*blockSize + yFactor: path[i]['y']*blockSize,
                path[i+1]['x']*blockSize, path[i+1]['y']*blockSize
            );
            ...
            if(Object.keys(path[i]).length>2){
                ...
                if(path[i]['pedido']>=0){
                    if(imgClientWarehouse)
                        p5.image(imgClientWarehouse,path[i]['x']*blockSize - 10, path[i]['y']*blockSize-10, 20,20);
                }
            }
        }
    }
    ...
    const renderTruck = (p5, route) => {
        ...
        if(curNode < route.path.length - 1){     
            ...
            if(route.path[curNode]['y'] < route.path[curNode+1]['y']){ // v 710,0,402,710
                downDirection=1; 
                sx = 710;
                sy = 0;
                sw = 402;
            ...
            if(route.path[curNode]['y'] > route.path[curNode+1]['y']){ // ^ 710,710,402,710
                upDirection=-1; 
                sx = 710;
                sy = 710;
                sw = 402;
        ...
        var xFactor = (rightDirection+leftDirection)*(distance - curNode*1000)*blockSize/1000;
        var yFactor = (upDirection+downDirection)*(distance - curNode*1000)*blockSize/1000;
    ...
    const renderRoadBlock = (p5, rb) => {
        ...
        for(var i=0; i<path.length-1;i++){
            p5.line(
                path[i]['x']*blockSize, path[i]['y']*blockSize,
                path[i+1]['x']*blockSize, path[i+1]['y']*blockSize
            );

            if(imgRoadblock){
                p5.image(imgRoadblock,path[i]['x']*blockSize - rbImageSize/2, path[i]['y']*blockSize-rbImageSize/2,rbImageSize,rbImageSize);
            }
        }
        if(imgRoadblock){
            p5.image(imgRoadblock,path[path.length-1]['x']*blockSize - rbImageSize/2, path[path.length-1]['y']*blockSize-rbImageSize/2,rbImageSize,rbImageSize);
        }
    }
    ...
    const draw = (p5) => {
        ...
        if(imgPlantaPrincipal){
            p5.image(imgPlantaPrincipal,12*blockSize - 20,8*blockSize-40,40,70);
        }

        if(imgPlantaSecundaria){
            p5.image(imgPlantaSecundaria,42*blockSize - 15,42*blockSize-12.5,30,25);
        }

        if(imgPlantaSecundaria){
            p5.image(imgPlantaSecundaria,63*blockSize - 15,3*blockSize-12.5,30,25);
        }
